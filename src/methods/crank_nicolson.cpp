...
void CrankNicolsonPortfolioOptimizer::initialize_optimization() {
...
double CrankNicolsonPortfolioOptimizer::iterate(
    ...
    const Matrix a_sum = rho1 * m_a_hx + rho2 * m_a_hx2;
    const Vector v_i = (m_im + a_sum).inverse() * (m_im - a_sum) * current_row;
    return v_i(col);
}