...
std::pair<Matrix, Matrix> CrankNicolsonPortfolioOptimizer::optimize(double yield, double interest_rate, double volatility) const {
    Matrix v = m_mesh;
    ...
    for (MatrixDimSizeType i = v.rows() - 1; i > 0; i--) {
        ...
        for (MatrixDimSizeType j = 1; j < v.cols(); j++) {
            ...
            for (double a = 0.2; a <= 0.9; a += 0.01) {
                ...
                const Matrix a_sum = rho1 * a_hx + rho2 * a_hx2;
                const Vector v_i = (im + a_sum).inverse() * (im - a_sum) * current_row;
